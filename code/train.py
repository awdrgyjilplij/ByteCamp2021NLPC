...
def main():
    ...
    model = BertForSequenceClassification.from_pretrained(pretrained, config=model_config, state_dict=state_dict)
    ...
    for ie in range(int(args.num_train_epochs)):
        ...
        for batch in tqdm(list(eval_loader)):
            batch = tuple(t.to(device) for t in batch)
            input_ids, attn_mask, labels = batch
        ...
        if eval_accuracy >= best_accuracy:
            torch.save(model.state_dict(), "model/model_best_unlabel.pt")
